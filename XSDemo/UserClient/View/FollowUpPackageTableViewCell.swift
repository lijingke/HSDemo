//
//  FollowUpPackageTableViewCell.swift
//  NGOFundManager
//
...
class FollowUpPackageTableViewCell: UITableViewCell {
    
    static var reuseId = "FollowUpPackageTableViewCell"
    ...
    public func configureData(_ entity: FollowUpPackageEntity) {
        ...
        if let price = entity.price {
            if price > 0 {
                bookPrice.setTitle("¥\(price) 预定", for: .normal)
            }else {
        ...
        if entity.isCollected == true {
            starBtn.setImage(UIImage(named: "star_y"), for: .normal)
        }else {
            starBtn.setImage(UIImage(named: "star_n"), for: .normal)
        }
        
    }
    
    @objc func btnAction(_ sender: UIButton) {
        delegate?.followUpPackageBtnAction(sender, entity: self.entity ?? FollowUpPackageEntity())
    }
    ...
    lazy var bookPrice: UIButton = {
        let btn = UIButton(type: .custom)
        btn.tag = 1000
        btn.setTitle("¥99.00 预定", for: .normal)
        btn.setTitleColor(UIColorFromRGB(rgbValue: 0xFF4919), for: .normal)
        btn.titleLabel?.font = UIFont.semibold(13)
        btn.backgroundColor = UIColor.init(hex: 0xFF4919, alpha: 0.1)
    ...
    lazy var starBtn: UIButton = {
        let btn = UIButton()
        btn.tag = 1001
        btn.setImage(UIImage(named: "star_y"), for: .normal)
        btn.addTarget(self, action: #selector(btnAction(_:)), for: .touchDown)
        return btn
    }()
...
extension FollowUpPackageTableViewCell {
    fileprivate func configureUI() {
        ...
        starBtn.snp.makeConstraints { (make) in
            make.centerY.equalTo(packageDetail)

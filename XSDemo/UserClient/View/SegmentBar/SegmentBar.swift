    ...
    func updateSegmentItemsStyle()  {
        ...
        if config.isItemEqualWidth { // 等分宽度
            ...
            for btn in itemBtns {
                ...
                btn.frame = CGRect(x: x + config.leftMargin, y: 0, width: btn.bounds.size.width + 10, height: config.barHeight - config.bottomLineHeight)
            ...
            itemsContentV.frame = CGRect(x: screenWidth / 2 - totalW / 2, y: 0, width: width, height: height)
...
//        self.bottomLine.bounds.size.width = width
        self.bottomLine.bounds.size.width = 24
        self.bottomLine.cornerWithRadius(redius: 2.5)
